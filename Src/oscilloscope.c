    ...
    for (;;)
    {
        ...
        if (__HAL_TIM_GET_COUNTER(&htim7) > 10000) {
            ...
            if (is_freq_captured && freqmeter_clk_count > 600) {
                ...
                LCD_DrawString(GRID_X + 64, GRID_Y + GRID_HEIGHT + 16, 24, str_buffer, PURPLE);
                is_freq_captured = 0;
            }
            else {
                LCD_DrawString(GRID_X + 64, GRID_Y + GRID_HEIGHT + 16, 24, "------", PURPLE);
            }
            /* 峰峰值显示 */
            if (volt_pp < 1000.0f) {
            ...
            LCD_DrawString(GRID_X + 280, GRID_Y + GRID_HEIGHT + 16, 24, str_buffer, PURPLE);
            ...
            LCD_DrawString(GRID_X + 472, GRID_Y + GRID_HEIGHT + 16, 24, str_buffer, PURPLE);
#if DEBUG
...
static void AdjustTriggerVoltage(_Bool up_down_select)
{
    ...
    Graph_DrawDashedLineY(&graph, osc_args.TriggerVolt * osc_args.DisplayScaleFactor + GRID_HEIGHT / 2 + osc_args.VoltOffset, AZURE);
